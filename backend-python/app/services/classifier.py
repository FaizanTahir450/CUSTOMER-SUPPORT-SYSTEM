# classifier.py
from app.services.llm import get_classification_llm
import logging
...
- "greeting_smalltalk" - Greetings, introductions, personal conversation, casual chat, remembering names
  Examples: "hi", "hello", "how are you", "my name is John", "do you remember my name?", "good morning", "what's your name?", "nice to meet you", "Thank you"
...
Respond with ONLY the category name, nothing else."""
    ...
    if classification not in ["greeting_smalltalk", "irrelevant", "relevant"]:
        logger.warning(f"Unexpected classification result: {classification}, defaulting to irrelevant")
        classification = "irrelevant"
    
    logger.debug(f"Classified message as '{classification}': {message[:50]}")
...
Respond with ONLY the category name, nothing else."""
    ...
    if classification not in ["company_info", "order_related"]:
        logger.warning(f"Unexpected sub-classification result: {classification}, defaulting to company_info")
        classification = "company_info"
    
    logger.debug(f"Sub-classified message as '{classification}': {message[:50]}")
    return classification