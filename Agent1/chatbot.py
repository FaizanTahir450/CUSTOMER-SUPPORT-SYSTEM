...
class LAMAChatbot:
    def __init__(self, vector_store, memory_manager):
        ...
        # Initialize Gemini model with SystemMessage conversion
...
Crucial Policy Note: If you find conflicting exchange periods (7 days in FAQ vs. 15 days in Policy), default to 7 days and note the discrepancy. Never make up information."""
        ...
        # Keep memory manager for later
        self.memory_manager = memory_manager
        self.agent_executor = None  # will be set later manually
    ...
    def ask(self, question):
        try:
            ...
            # If response is a dict (AgentExecutor output)
            if isinstance(response, dict) and "output" in response:
                return response["output"]
            
            # If response is a list of messages
            if isinstance(response, list):
                return " ".join([msg.content for msg in response if isinstance(msg, AIMessage)]
                                or [str(msg) for msg in response])
            
            # If response is a single AIMessage
