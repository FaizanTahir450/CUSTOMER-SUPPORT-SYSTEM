    ...
    try:
        ...
        with st.chat_message("assistant"):
            with st.spinner("Thinking..."):
                ...
                cached_answer = search_similar_question(cleaned_input)
                if cached_answer:
                    answer = cached_answer
                else:
                    # Step 2: Retrieve from vector store & run LLM
                    reviews = retreiver.invoke(cleaned_input)
                    answer_obj = chain.invoke({"reviews": reviews, "question": cleaned_input})
                    answer = answer_obj.content if hasattr(answer_obj, "content") else str(answer_obj)

                    # Step 3: Store in semantic cache
                    add_to_semantic_cache(cleaned_input, answer)
